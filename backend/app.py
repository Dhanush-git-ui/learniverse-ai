...
def type_aware_compare(expected, actual, output_type: str = "auto") -> bool:
    ...
    if output_type in ["integer_array", "string_array", "matrix", "array", "list"] or exp_str.startswith("["):
        ...
        act_parsed = None
        try:
            exp_parsed = ast.literal_eval(exp_str) if isinstance(expected, str) else expected
        except Exception:
            try:
        ...
        try:
            act_parsed = ast.literal_eval(act_str) if isinstance(actual, str) else actual
        except Exception:
            try:
                act_parsed = json.loads(act_str)
            except Exception:
                pass
