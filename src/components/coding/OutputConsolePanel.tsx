          ...
          <div className="space-y-4">
            {!result ? (
              ...
              <div className="space-y-4">
                ...
                {/* Per-test-case diff (submit mode) */}
                {result.results && result.passed_cases !== undefined && (
                  <div className="space-y-2 mt-1">
                    {result.results.map((r: any, i: number) => (
                      ...
                      >
                        ...
                        <p><span className="text-slate-400">Input:    </span>{r.input}</p>
                        <p><span className="text-slate-400">Expected: </span><span className="text-emerald-400">{r.expected}</span></p>
                        <p><span className="text-slate-400">Got:      </span><span className={r.passed ? 'text-emerald-400' : 'text-rose-400'}>{r.actual || '(no output)'}</span></p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {result.results && result.results[0] && result.passed_cases === undefined && (
                  <div className="p-3.5 bg-rose-950/20 border-l-2 border-rose-600 text-rose-300 font-mono whitespace-pre-wrap text-[11px] leading-relaxed">
                    <strong className="block mb-1 text-rose-400">Compilation / Runtime Error:</strong>
...
=======
import { Terminal, CheckCircle2, XCircle, Sparkles, AlertCircle } from 'lucide-react';
